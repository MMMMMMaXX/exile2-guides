...
console.log(
  "sourceContentId=",
  d.translation.sourceContentId,
  "| sourceRevision=",
  d.translation.sourceRevision,
  "| sourceLocale=",
  d.translation.sourceLocale,
);
const forbiddenRoot = ["contentId", "sourceContentId", "sourceRevision"].filter(
  (k) => k in d,
);
console.log(
  "forbidden root keys present=",
  forbiddenRoot.length ? forbiddenRoot : "none",
);
["completed", "manual-review", "none"].forEach((w) => {
  if (JSON.stringify(d).includes('"' + w + '"'))
    console.log("FORBIDDEN WORD FOUND:", w);
});
