...
Arbiter of Ash 被列为首发研究候选，是因为当前 Patch Notes 可能影响终局遭遇行为。本文不保留任何旧版本的阶段顺序、伤害假设或走位建议。

## 观察计划

1. 保存当前遭遇名称、入场路径和角色环境。
2. 只有在观察到时才记录每个阶段转换，包括可见触发与恢复结果。
3. 在官方或游戏内命名确认前，按预警和观察到的结果描述攻击。
