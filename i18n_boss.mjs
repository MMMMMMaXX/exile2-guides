...
const TRANSLATABLE_KEYS = new Set([
  'title', 'summary', 'description', 'imageAlt',
  'callout', 'calloutDetail', 'label', 'text', 'body', 'note',
  'why', 'fix', 'checks', 'objectives', 'notes', 'trigger', 'name',
  'telegraph', 'responses', 'commonMistakes', 'paragraphs', 'bullets',
  'question', 'summary', 'editorialAnalysis', 'linkLabel', 'symptom',
  'directAnswer', 'condition', 'changes', 'answer', 'alt', 'caption'
]);
...
function modeCollect(slug) {
  const en = JSON.parse(fs.readFileSync(path.join(EN_DIR, `${slug}.json`), 'utf8'));
  const set = new Map();
  walk(en, (k, v) => {
    const key = v;
...
function isUrlLike(v) {
  return typeof v === 'string' && (v.includes('http') || v.startsWith('/') || v.startsWith('#') || v.startsWith('www.'));
}
...
function modeApply(slug, ...mapPaths) {
  ...
  fs.writeFileSync(path.join(DE_DIR, `${slug}.json`), JSON.stringify(de, null, 2) + '\n');
  console.log(`Wrote ${slug}.json (applied ${applied}, left English ${missing})`);
}
...
if (mode === 'collect') modeCollect(slug);
  ...
  console.error('Usage: node i18n_boss.mjs collect <slug>  |  node i18n_boss.mjs apply <slug> <map.json> [base.json ...]');
  process.exit(1);
}
