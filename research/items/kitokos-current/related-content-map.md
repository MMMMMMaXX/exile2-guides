# Related Content Map — Kitoko's Current

Checked date: 2026-08-03
Verification status: pending-pc

Maps the related-* IDs used in the article's `related-content` section to allowlisted,
published slugs. All IDs below were verified to exist in the content tree (skills/guides/
items/builds/patches). No unallowlisted slug is linked.
...
- `ingenuity` was considered (plan §14 suggested it for dream-fragments) but is NOT in the
  published allowlist of item slugs, so it is deliberately not linked from either article.
- All linked slugs above were confirmed present via directory listing of the content tree.

## Uncertain / flagged

- If any linked slug is later unpublished, the related-content section must be trimmed to
  avoid dangling links. Re-verify at publish time.
