# Source Ledger — Expedition Logbooks
...
- Ocean Biome reveal via Atlas (not old Logbook UI / Realmgate): supported by Wiki + Game8 + plan. Current 0.5 (pending in-game verification) for exact button position.
- Runic Splinter disabled in 0.5: stated in plan SECTION 8 / current-vs-legacy. Historical only.
- Styrn guaranteed Logbook drop: community guide (0.5.3); mark as current 0.5 (pending in-game verification).
- No fixed drop-rate figures used (single-sample rates avoided per plan gate #5).
