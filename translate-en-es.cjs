...
function isNum(s) {
  return (
    /^\s*-?\d+([.,]\d+)?%?\s*$/.test(s) ||
    /^\s*\d+(\.\d+)?\s*(m|km|lvl|level|%|x)\s*$/i.test(s.trim())
  );
}
function isIdKey(k) {
  return /Id$/i.test(k) || /Ids$/i.test(k);
}
function rewriteLinks(s) {
  return s.replace(/\/en\//g, "/es/");
}
...
function classify(key, val, ctx) {
  ...
  if ((ctx === "sources" || ctx === "categories") && key === "label")
    return null;
  if (!PROSE_KEYS.has(key)) return null;
  if (
    ENUM_SINGLE.has(val.trim().toLowerCase()) &&
...
function forEach(obj, ctx, visit) {
  if (Array.isArray(obj)) {
    obj.forEach((item, i) => {
      if (item && typeof item === "object") forEach(item, ctx, visit);
    });
    return;
  }
...
function main() {
  ...
  if (mode === "apply") {
    ...
    forEach(clone, "root", ({ get, set, action }) => {
      const en = get();
      if (i >= esArr.length) {
        missing.push("Ran out of translations at: " + en.slice(0, 60));
        return;
      }
      const es = esArr[i++];
    ...
    clone.locale = "es";
    if (i !== esArr.length)
      missing.push(`Count mismatch: en=${i} esArr=${esArr.length}`);
  ...
  if (mode === "batch") {
    ...
    for (const enPath of Object.keys(manifest)) {
      ...
      forEach(clone, "root", ({ get, set, action }) => {
        const e = get();
        if (i >= esArr.length) {
          miss.push("out of translations: " + e.slice(0, 50));
          return;
        }
        const es = esArr[i++];
        if (action === "linkonly") set(rewriteLinks(e));
        else set(rewriteLinks(es));
      });
      clone.locale = "es";
      if (i !== esArr.length)
        miss.push("count mismatch en=" + i + " es=" + esArr.length);
      if (miss.length) {
        problems++;
        console.log("PROBLEM " + enPath);
        miss.slice(0, 5).forEach((m) => console.log("   " + m));
      } else {
        total++;
