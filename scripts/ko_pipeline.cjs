...
function check(p, d) {
  ...
  if (/TODO|草稿|draft|placeholder|REPLACE_WITH/i.test(s))
    bad.push("forbidden-marker");
...
function main() {
  ...
  if (cmd === "extract") {
    ...
    if (idx.i !== trs.length) {
      console.error(
        "WARN translation count mismatch: applied",
        idx.i,
        "expected",
        trs.length,
      );
    ...
    console.error(
      "usage: ko_pipeline.cjs extract <en> <stringsOut> | apply <en> <translations> <koOut>",
    );
    process.exit(2);
  }
}
