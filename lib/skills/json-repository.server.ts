/** 文件职责：在构建阶段发现、解析并校验 Skills JSON，浏览器运行时不得直接读取文件系统。 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import type { ZodIssue } from "zod";

import { InMemorySkillRepository } from "./repository";
import { skillArticleSchema, type SkillArticle } from "./schema";

export type SkillJsonIssue = {
  code:
    | "invalid-json"
    | "invalid-schema"
    | "duplicate-id-locale"
...
export class SkillJsonError extends Error {
  readonly issues: readonly SkillJsonIssue[];

  /** 保存全部 JSON 问题，使 CLI 能一次向内容编辑者报告所有待修文件。 */
  constructor(issues: readonly SkillJsonIssue[]) {
    super(`Skill JSON validation failed with ${issues.length} issue(s)`);
    this.name = "SkillJsonError";
    this.issues = issues;
  }
}

/** 递归发现位于任意语言 skills 目录中的 JSON，兼容生产内容和隔离测试目录。 */
export async function discoverSkillJsonFiles(
  ...
  const files = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return discoverSkillJsonFiles(entryPath);
      const segments = entryPath.split(path.sep);
      return entry.name.endsWith(".json") && segments.includes("skills")
        ? [entryPath]
        : [];
    }),
  );
...
function schemaIssue(issue: ZodIssue, sourcePath: string): SkillJsonIssue {
  return {
    code: "invalid-schema",
...
/**
 * 加载全部 Skill JSON 并一次报告跨文件冲突。
 * 文件名必须等于 slug，防止内容移动或导入数据库时出现隐含别名。
 */
export async function loadSkillArticles(
  contentDirectory = path.resolve(process.cwd(), "content"),
  projectRoot = process.cwd(),
): Promise<SkillArticle[]> {
  const files = await discoverSkillJsonFiles(contentDirectory);
  const articles: { article: SkillArticle; sourcePath: string }[] = [];
  const issues: SkillJsonIssue[] = [];
  ...
  for (const file of files) {
    ...
    const result = skillArticleSchema.safeParse(value);
    if (!result.success) {
      issues.push(
  ...
  for (const entry of articles) {
    ...
    if (existingRoute) {
      issues.push({
        ...
        message: `/${entry.article.locale}/skills/${entry.article.slug}/ is declared more than once`,
    ...
    for (const translation of group.slice(1)) {
      if (translation.article.slug !== reference.article.slug) {
        issues.push({
          ...
          message: `translations for ${reference.article.id} must share one stable slug`,
  ...
  if (issues.length > 0) throw new SkillJsonError(issues);
  return articles.map((entry) => entry.article);
}

/** 创建当前 JSON 数据源仓储；未来数据库实现只需遵守 SkillRepository。 */
export async function createJsonSkillRepository(
  contentDirectory?: string,
): Promise<InMemorySkillRepository> {
  return new InMemorySkillRepository(await loadSkillArticles(contentDirectory));
}
