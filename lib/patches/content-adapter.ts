/** 文件职责：把 PatchArticle 投影到现有通用内容索引，保持搜索、SEO 和关联模块只有一套公共数据链。 */
...
/** 生成供通用内容消费者读取的 Front Matter；Patch 完整结构仍保留在 patchArticle。 */
export function patchArticleToFrontMatter(
...
function patchIndexBody(article: PatchArticle): string {
...
/** 将 PatchArticle 接入现有内容索引，同时保留原始结构供详情页渲染。 */
export function patchArticleToParsedContent(
  article: PatchArticle,
  sourcePath = `content/${article.locale}/patches/${article.slug}.json`,
): ParsedContent {
  return {
    body: patchIndexBody(article),
    extension: ".json",
    frontMatter: patchArticleToFrontMatter(article),
