/** 文件职责：在构建阶段发现、解析并校验 Patches JSON，浏览器运行时不得直接读取文件系统。 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import type { ZodIssue } from "zod";

import { InMemoryPatchRepository } from "./repository";
import { patchArticleSchema, type PatchArticle } from "./schema";

export type PatchJsonIssue = {
  code:
    | "invalid-json"
    | "invalid-schema"
    | "duplicate-id-locale"
...
export class PatchJsonError extends Error {
  readonly issues: readonly PatchJsonIssue[];

  /** 保存全部 JSON 问题，使 CLI 能一次向内容编辑者报告所有待修文件。 */
  constructor(issues: readonly PatchJsonIssue[]) {
    super(`Patch JSON validation failed with ${issues.length} issue(s)`);
    this.name = "PatchJsonError";
    this.issues = issues;
  }
}

/** 递归发现位于任意语言 patches 目录中的 JSON，兼容生产内容和隔离测试目录。 */
export async function discoverPatchJsonFiles(
  ...
  const files = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return discoverPatchJsonFiles(entryPath);
      const segments = entryPath.split(path.sep);
      return entry.name.endsWith(".json") && segments.includes("patches")
        ? [entryPath]
        : [];
    }),
  );
...
function schemaIssue(issue: ZodIssue, sourcePath: string): PatchJsonIssue {
  return {
    code: "invalid-schema",
...
/**
 * 加载全部 Patch JSON 并一次报告跨文件冲突。
 * 文件名必须等于 slug，防止内容移动或导入数据库时出现隐含别名。
 */
export async function loadPatchArticles(
  contentDirectory = path.resolve(process.cwd(), "content"),
  projectRoot = process.cwd(),
): Promise<PatchArticle[]> {
  const files = await discoverPatchJsonFiles(contentDirectory);
  const articles: { article: PatchArticle; sourcePath: string }[] = [];
  const issues: PatchJsonIssue[] = [];
  ...
  for (const file of files) {
    ...
    const result = patchArticleSchema.safeParse(value);
    if (!result.success) {
      issues.push(
  ...
  for (const entry of articles) {
    ...
    if (existingRoute) {
      issues.push({
        ...
        message: `/${entry.article.locale}/patches/${entry.article.slug}/ is declared more than once`,
  ...
  if (issues.length > 0) throw new PatchJsonError(issues);
  return articles.map((entry) => entry.article);
}

/** 创建当前 JSON 数据源仓储；未来数据库实现只需遵守 PatchRepository。 */
export async function createJsonPatchRepository(
  contentDirectory?: string,
): Promise<InMemoryPatchRepository> {
  return new InMemoryPatchRepository(await loadPatchArticles(contentDirectory));
}
