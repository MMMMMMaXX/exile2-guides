/** 文件职责：定义 Patches 数据源抽象；页面和服务不得依赖 JSON 或未来数据库的具体读取方式。 */
...
export interface PatchRepository {
  ...
  getPublished(locale?: ContentLocale): Promise<readonly PatchArticle[]>;
}

/** 内存实现承接已校验数据；JSON 与未来 API 适配器均可复用相同查询语义。 */
export class InMemoryPatchRepository implements PatchRepository {
  readonly #articles: readonly PatchArticle[];

  /** 复制已校验输入，避免外部数组后续变更影响同一次构建结果。 */
  constructor(articles: readonly PatchArticle[]) {
    this.#articles = [...articles];
  }

  /** 返回全部编辑记录；发布过滤由明确的公开查询承担。 */
  async getAll(): Promise<readonly PatchArticle[]> {
  ...
  async getPublished(locale?: ContentLocale): Promise<readonly PatchArticle[]> {
