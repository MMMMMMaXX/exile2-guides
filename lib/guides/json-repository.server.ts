/** 文件职责：在构建阶段发现、解析并校验 Guides JSON，浏览器运行时不得直接读取文件系统。 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import type { ZodIssue } from "zod";

import { InMemoryGuideRepository } from "./repository";
import { guideArticleSchema, type GuideArticle } from "./schema";

export type GuideJsonIssue = {
  code:
    | "invalid-json"
    | "invalid-schema"
    | "duplicate-id-locale"
...
export class GuideJsonError extends Error {
  readonly issues: readonly GuideJsonIssue[];

  /** 保存全部 JSON 问题，使 CLI 能一次向内容编辑者报告所有待修文件。 */
  constructor(issues: readonly GuideJsonIssue[]) {
    super(`Guide JSON validation failed with ${issues.length} issue(s)`);
    this.name = "GuideJsonError";
    this.issues = issues;
  }
}

/** 递归发现位于任意语言 guides 目录中的 JSON，兼容生产内容和隔离测试目录。 */
export async function discoverGuideJsonFiles(
  ...
  const files = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return discoverGuideJsonFiles(entryPath);
      const segments = entryPath.split(path.sep);
      return entry.name.endsWith(".json") && segments.includes("guides")
        ? [entryPath]
        : [];
    }),
  );
...
function schemaIssue(issue: ZodIssue, sourcePath: string): GuideJsonIssue {
  return {
    code: "invalid-schema",
...
export async function loadGuideArticles(
  ...
  const files = await discoverGuideJsonFiles(contentDirectory);
  const articles: { article: GuideArticle; sourcePath: string }[] = [];
  const issues: GuideJsonIssue[] = [];
  ...
  for (const file of files) {
    ...
    const result = guideArticleSchema.safeParse(value);
    if (!result.success) {
      issues.push(
  ...
  for (const entry of articles) {
    ...
    if (existingRoute) {
      issues.push({
        ...
        message: `/${entry.article.locale}/guides/${entry.article.slug}/ is declared more than once`,
  ...
  if (issues.length > 0) throw new GuideJsonError(issues);
  return articles.map((entry) => entry.article);
}

/** 创建当前 JSON 数据源仓储；未来数据库实现只需遵守 GuideRepository。 */
export async function createJsonGuideRepository(
  contentDirectory?: string,
): Promise<InMemoryGuideRepository> {
  return new InMemoryGuideRepository(await loadGuideArticles(contentDirectory));
}
