/** 文件职责：把 GuideArticle 投影到现有通用内容索引，保持搜索、SEO 和关联模块只有一套公共数据链。 */
...
/** 生成供通用内容消费者读取的 Front Matter；Guide 完整结构仍保留在 guideArticle。 */
export function guideArticleToFrontMatter(
  ...
  return contentFrontMatterSchema.parse({
    ...
    guideCategory: article.guideCategory,
...
function guideIndexBody(article: GuideArticle): string {
...
/** 将 GuideArticle 接入现有内容索引，同时保留原始结构供详情页渲染。 */
export function guideArticleToParsedContent(
  article: GuideArticle,
  sourcePath = `content/${article.locale}/guides/${article.slug}.json`,
): ParsedContent {
  return {
    body: guideIndexBody(article),
    extension: ".json",
    frontMatter: guideArticleToFrontMatter(article),
