/** 文件职责：定义 Guides 数据源抽象；页面和服务不得依赖 JSON 或未来数据库的具体读取方式。 */
...
export interface GuideRepository {
  ...
  getPublished(locale?: ContentLocale): Promise<readonly GuideArticle[]>;
}
...
export class InMemoryGuideRepository implements GuideRepository {
  ...
  constructor(articles: readonly GuideArticle[]) {
    this.#articles = [...articles];
  }

  /** 返回全部编辑记录；发布过滤由明确的公开查询承担。 */
  async getAll(): Promise<readonly GuideArticle[]> {
  ...
  async getPublished(locale?: ContentLocale): Promise<readonly GuideArticle[]> {
