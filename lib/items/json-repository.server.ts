/** 文件职责：在构建阶段发现、解析并校验 Items JSON，浏览器运行时不得直接读取文件系统。 */
import { readdir, readFile } from "node:fs/promises";
import path from "node:path";
import type { ZodIssue } from "zod";

import { InMemoryItemRepository } from "./repository";
import { itemArticleSchema, type ItemArticle } from "./schema";

export type ItemJsonIssue = {
  code:
    | "invalid-json"
    | "invalid-schema"
    | "duplicate-id-locale"
...
export class ItemJsonError extends Error {
  readonly issues: readonly ItemJsonIssue[];

  /** 保存全部 JSON 问题，使 CLI 能一次向内容编辑者报告所有待修文件。 */
  constructor(issues: readonly ItemJsonIssue[]) {
    super(`Item JSON validation failed with ${issues.length} issue(s)`);
    this.name = "ItemJsonError";
    this.issues = issues;
  }
}

/** 递归发现位于任意语言 items 目录中的 JSON，兼容生产内容和隔离测试目录。 */
export async function discoverItemJsonFiles(
  ...
  const files = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) return discoverItemJsonFiles(entryPath);
      const segments = entryPath.split(path.sep);
      return entry.name.endsWith(".json") && segments.includes("items")
        ? [entryPath]
        : [];
    }),
  );
...
function schemaIssue(issue: ZodIssue, sourcePath: string): ItemJsonIssue {
  return {
    code: "invalid-schema",
...
/**
 * 加载全部 Item JSON 并一次报告跨文件冲突。
 * 文件名必须等于 slug，防止内容移动或导入数据库时出现隐含别名。
 */
export async function loadItemArticles(
  contentDirectory = path.resolve(process.cwd(), "content"),
  projectRoot = process.cwd(),
): Promise<ItemArticle[]> {
  const files = await discoverItemJsonFiles(contentDirectory);
  const articles: { article: ItemArticle; sourcePath: string }[] = [];
  const issues: ItemJsonIssue[] = [];
  ...
  for (const file of files) {
    ...
    const result = itemArticleSchema.safeParse(value);
    if (!result.success) {
      issues.push(
  ...
  for (const entry of articles) {
    ...
    if (existingRoute) {
      issues.push({
        ...
        message: `/${entry.article.locale}/items/${entry.article.slug}/ is declared more than once`,
  ...
  if (issues.length > 0) throw new ItemJsonError(issues);
  return articles.map((entry) => entry.article);
}

/** 创建当前 JSON 数据源仓储；未来数据库实现只需遵守 ItemRepository。 */
export async function createJsonItemRepository(
  contentDirectory?: string,
): Promise<InMemoryItemRepository> {
  return new InMemoryItemRepository(await loadItemArticles(contentDirectory));
}
