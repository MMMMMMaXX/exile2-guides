...
const resultImages: Record<ContentType, string> = {
...
export function SearchPage({
  ...
  const labels: Record<ContentType | "all", string> = zh
    ? {
        all: "全部",
        boss: "首领",
        build: "Build",
          ...
          {results.length ? (
            <div>
              {results.map((document) => (
                <a
                  className="v4-prototype-search-result"
                  href={document.path}
                ...
                >
                  <img
                    ...
                    src={resultImages[document.category]}
                    srcSet={`${resultImages[document.category]} 1400w`}
                    width="1400"
                  />
                  <span>
                    <small>{labels[document.category]}</small>
